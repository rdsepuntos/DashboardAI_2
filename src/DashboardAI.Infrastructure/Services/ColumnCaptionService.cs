...
namespace DashboardAI.Infrastructure.Services
{
    ...
    public class ColumnCaptionService : IColumnCaptionService
    {
        ...
        public async Task<IReadOnlyDictionary<string, string>> GetCaptionsAsync(string dataSourceName, int storeId)
        {
            ...
            try
            {
                var cacheKey = storeId + "|" + dataSourceName;
                if (_cache.TryGetValue(cacheKey, out var cached))
                    return cached;

                using (var conn = new SqlConnection(_connectionString))
                {
                    var captions = await LoadCaptionsAsync(conn, mapping, parentPageId, ucPageId, storeId);
        ...
        // Informational only — spPageFields_New resolves the member from @StoreID internally.
        ...
        public async Task<CaptionDiagnostics> GetDiagnosticsAsync(string dataSourceName, int storeId)
        {
            ...
            try
            {
                using (var conn = new SqlConnection(_connectionString))
                {
                    // Informational — the proc resolves the member from @StoreID itself.
                    diag.MemberId = await ResolveMemberIdAsync(conn, storeId);

                    var captions   = await LoadCaptionsAsync(conn, mapping, diag.ParentPageId, diag.UCPageId, storeId);
        ...
        private async Task<IReadOnlyDictionary<string, string>> LoadCaptionsAsync(
            SqlConnection conn, PageFieldMapping mapping, int parentPageId, int ucPageId, int storeId)
        {
            ...
            foreach (var row in rows)
            {
                ...
                // TableColumnID holds the physical view column name; ColCaption is its display label.
                if (!dict.TryGetValue("TableColumnID", out var colObj) || colObj == null) continue;
