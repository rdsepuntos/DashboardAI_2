...
/****** Object:  View [dbo].[AID_RapidRiskAssessor]    Script Date: 9/24/2026 1:50:03 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dbo].[AID_RapidRiskAssessor]
AS
SELECT        A.StoreID, A.RegOthID, A.InternalNo, A.TitleDesc, ISNULL(sts.StatusDesc, 'N/A') AS Status, A.StartDt, ISNULL(rt.RegRecDesc, 'N/A') AS Type, ISNULL(rst.SubTypeDesc, 'N/A') AS SubType, ISNULL(loctype.LocationTypeName, 'N/A') AS LocationType, ISNULL(A.LocationName, 'N/A') 
                         AS Location, ISNULL(chk.TemplateName, 'N/A') AS Checklist, A.CreatedDt AS CreatedDate, ISNULL(A.CreatedByName, 'N/A') AS CreatedBy, 
                         ISNULL(presp.FirstName + ' ' + presp.LastName, 'N/A') AS PersonResponsible,
                         ISNULL(division.DivDeptName, 'N/A') AS Division, ISNULL(department.DivDeptName, 
...
FROM            dbo.RegisterOthHdr AS A LEFT OUTER JOIN
						 ...
						 dbo.Store ST ON ST.StoreID = A.StoreID
